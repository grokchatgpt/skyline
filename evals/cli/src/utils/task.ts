...
/**
 * Send a task to the skyline test server
 * @param task The task description to send
 * @param apiKey Optional skyline API key to use for the task
...
export async function sendTaskToServer(task: string, apiKey?: string): Promise<any> {
	...
	try {
		...
		if (error.code === "ECONNREFUSED") {
			throw new Error(
				"Could not connect to the test server. Make sure VSCode is running with the skyline extension and the test server is active.",
			)
		}

...
import { BrowserAction, BrowserActionResult, skylineMessage, skylineSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: skylineMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: skylineMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: skylineMessage[] = []
		let nextActionMessages: skylineMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					<>{isAutoApproved ? "skyline is using the browser:" : "skyline wants to use the browser:"}</>
				</span>
			</div>
			<div
				style={{

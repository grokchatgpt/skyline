...
interface ChatRowProps {
	...
	isExpanded: boolean
	onToggleExpand: () => void
	lastModifiedMessage?: skylineMessage
	isLast: boolean
...
export const ChatRowContent = ({
	...
	const [cost, apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
		if (message.text != null && message.say === "api_req_started") {
			const info: skylineApiReqInfo = JSON.parse(message.text)
		...
		switch (type) {
			...
			case "mistake_limit_reached":
				return [
					...
					<span style={{ color: errorColor, fontWeight: "bold" }}>skyline is having trouble...</span>,
				]
			...
			case "command":
				return [
					...
					<span style={{ color: normalColor, fontWeight: "bold" }}>skyline wants to execute this command:</span>,
				]
			case "use_mcp_server":
				const mcpServerUse = JSON.parse(message.text || "{}") as skylineAskUseMcpServer
				return [
					...
					<span className="ph-no-capture" style={{ color: normalColor, fontWeight: "bold", wordBreak: "break-word" }}>
						skyline wants to {mcpServerUse.type === "use_mcp_tool" ? "use a tool" : "access a resource"} on the{" "}
			...
			case "followup":
				return [
					...
					<span style={{ color: normalColor, fontWeight: "bold" }}>Skyline has a question:</span>,
				]
			default:
				return [null, null]
		}
	...
	const tool = useMemo(() => {
		if (message.ask === "tool" || message.say === "tool") {
			return JSON.parse(message.text || "{}") as skylineSayTool
		}
		return null
				...
				return (
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>skyline wants to edit this file:</span>
						</div>
						<CodeAccordian
				...
				return (
					<>
						<div style={headerStyle}>
							...
							{tool.operationIsLocatedInWorkspace === false &&
								toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
							<span style={{ fontWeight: "bold" }}>skyline wants to create a new file:</span>
						</div>
						<CodeAccordian
							isLoading={message.partial}
							code={tool.content!}
					...
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{/* {message.type === "ask" ? "" : "skyline read this file:"} */}
								skyline wants to read this file:
							</span>
						</div>
						<div
							style={{
				...
				return (
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? "skyline wants to view the top level files in this directory:"
									: "skyline viewed the top level files in this directory:"}
							</span>
						</div>
						<CodeAccordian
							code={tool.content!}
				...
				return (
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? "skyline wants to recursively view all files in this directory:"
									: "skyline recursively viewed all files in this directory:"}
							</span>
						</div>
						<CodeAccordian
							code={tool.content!}
				...
				return (
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? "skyline wants to view source code definition names used in this directory:"
									: "skyline viewed source code definition names used in this directory:"}
							</span>
						</div>
						<CodeAccordian
							code={tool.content!}
					...
					<>
						<div style={headerStyle}>
							...
							{tool.operationIsLocatedInWorkspace === false &&
								toolIcon("sign-out", "yellow", -90, "This is outside of your workspace")}
							<span style={{ fontWeight: "bold" }}>
								skyline wants to search this directory for <code>{tool.regex}</code>:
							</span>
						</div>
						<CodeAccordian
							code={tool.content!}
	...
	if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
		const useMcpServer = JSON.parse(message.text || "{}") as skylineAskUseMcpServer
		...
		case "say":
			switch (message.say) {
											...
											<p
												...
												{apiRequestFailedMessage?.toLowerCase().includes("powershell") && (
													<>
														...
														<a
															href="https://github.com/skyline/skyline/wiki/TroubleShooting-%E2%80%90-%22PowerShell-is-not-recognized-as-an-internal-or-external-command%22"
															style={{
																color: "inherit",
				...
				case "user_feedback_diff":
					const tool = JSON.parse(message.text || "{}") as skylineSayTool
					return (
						<div
							style={{
								marginTop: -10,
					...
					return (
						<>
							<div
								...
								<div>
									skyline tried to access <code>{message.text}</code> which is blocked by the{" "}
									<code>.skylineignore</code>
									file.
								</div>
							</div>
						</>
					...
					return (
						<>
							<div
								...
								<div>
									skyline won't be able to view the command's output. Please update VSCode (
									<code>CMD/CTRL + Shift + P</code> → "Update") and make sure you're using a supported shell:
									zsh, bash, fish, or PowerShell (<code>CMD/CTRL + Shift + P</code> → "Terminal: Select Default
									Profile").{" "}
									<a
										href="https://github.com/skyline/skyline/wiki/Troubleshooting-%E2%80%90-Shell-Integration-Unavailable"
										style={{
											color: "inherit",
											textDecoration: "underline",
										}}>
		...
		case "ask":
			switch (message.ask) {
				...
				case "followup":
					...
					try {
						const parsedMessage = JSON.parse(message.text || "{}") as skylineAskQuestion
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ color: normalColor, fontWeight: "bold" }}>skyline wants to start a new task:</span>
							</div>
						...
						<>
							<div style={headerStyle}>
								...
								<span style={{ color: normalColor, fontWeight: "bold" }}>
									skyline wants to condense your conversation:
								</span>
							</div>
							<NewTaskPreview context={message.text || ""} />
						</>
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ color: normalColor, fontWeight: "bold" }}>
									skyline wants to create a Github issue:
								</span>
							</div>
				...
				case "plan_mode_respond": {
					...
					let selected: string | undefined
					try {
						const parsedMessage = JSON.parse(message.text || "{}") as skylinePlanModeResponse

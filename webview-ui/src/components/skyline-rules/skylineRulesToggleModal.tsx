...
const SkylineRulesToggleModal: React.FC = () => {
	const {
	...
	useEffect(() => {
		if (isVisible) {
			vscode.postMessage({ type: "refreshskylineRules" })
		}
	}, [isVisible])

	// Format global rules for display with proper typing
	const globalRules = Object.entries(globalskylineRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localskylineRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	return (
		<div ref={modalRef}>
			<div ref={buttonRef} className="inline-flex min-w-0 max-w-full">
				<Tooltip tipText="Manage skyline Rules & Workflows" visible={isVisible ? false : undefined}>
					<VSCodeButton
						appearance="icon"
						aria-label="skyline Rules"
			...
			{isVisible && (
				<div
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rules" ? (
							<p>
								Rules allow you to provide skyline with system-level guidance. Think of them as a persistent way
								to include context and preferences for your projects or globally for every conversation.
							</p>
						) : (
							<p>
								Workflows allow you to define a series of steps to guide skyline through a repetitive set of
...
export default SkylineRulesToggleModal

...
description: "Learn about the two primary transport mechanisms for communication between skyline and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between skyline and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (skyline) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as skyline
...
-   Single-client scenarios (one skyline instance per server)
...
1. The client (skyline) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your skyline instance
...
STDIO servers run locally on the same machine as skyline, which has several important implications:
...
-   **Execution**: Starts and stops with skyline (child process lifecycle)
...
-   Start when needed by skyline
-   Not require network configuration
-   Need to be installed alongside skyline or via a package manager
...
## Configuring Transports in skyline

For detailed information on configuring STDIO and SSE transports in skyline, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).

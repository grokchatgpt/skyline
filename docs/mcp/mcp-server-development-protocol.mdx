...
description: "This protocol is designed to streamline the development process of building MCP servers with skyline."
---

> 🚀 **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [skyline MCP Marketplace](https://github.com/skyline/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like skyline by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.skylinerules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/skyline/MCP).

### Using `.skylinerules` Files

A `.skylinerules` file is a special configuration that skyline reads automatically when working in the directory where it's placed. These files:

-   Configure skyline's behavior and enforce best practices
-   Switch skyline into a specialized MCP development mode
...
Here's the complete MCP Server Development Protocol that should be placed in your `.skylinerules` file:
...
When this `.skylinerules` file is present in your working directory, skyline will:
...
### 1. Create a `.skylinerules` file (🚨 IMPORTANT)

First, add a `.skylinerules` file to the root of your MCP working directory using the protocol above. This file configures skyline to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your skyline chat by clearly describing what you want to build. Be specific about:
...
skyline will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help skyline build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves skyline's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with skyline to design your MCP server:
...
Once planning is complete, skyline helps implement the server:
...
<Frame>
	<img
		src="https://storage.googleapis.com/skyline_public_images/docs/assets/planning-phase.gif"
...
<Frame>
	<img
		src="https://storage.googleapis.com/skyline_public_images/docs/assets/building-mcp-plugin.gif"
...
Resources let your MCP servers expose data to skyline without executing code. They're perfect for providing context like files, API responses, or database records that skyline can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing skyline to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).

...
description: "The Model Context Protocol (MCP) allows skyline to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
To access the MCP Servers interface in skyline:

1. Click on the skyline icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the skyline panel
...
4. skyline will attempt to connect to the server and display the connection status
...
> **Warning**: The following third-party marketplaces are listed for informational purposes only. skyline does not endorse, verify, or take responsibility for any servers listed on these marketplaces. These servers are cloud-hosted services that process your requests and may have access to data you share with them. Always review privacy policies and terms of use before connecting to third-party services.
...
2. **Request Timeout**:
    - Set how long skyline should wait for server responses
...
-   **Enabled**: skyline can use the server's tools and resources
...
For advanced users, skyline stores MCP server configurations in a JSON file that can be modified:
...
Once connected, skyline can use the tools and resources provided by the MCP server. When skyline suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to skyline

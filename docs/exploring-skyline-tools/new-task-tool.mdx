...
skyline includes a powerful internal tool, `new_task`, designed to help manage workflow continuity and context preservation, especially during complex or long-running tasks. This tool, combined with skyline's awareness of its own context window usage and the flexibility of `.skylinerules`, enables sophisticated strategies for breaking down work and ensuring seamless transitions between task sessions.
...
1. **The `new_task` Tool:**
    - **Function:** Allows skyline, upon user approval, to end the current task session and immediately start a new one.
    - **Context Preloading:** Crucially, skyline can **preload** this new task session with specific context provided within the tool's `<context>` block. This context can be anything skyline or a `.skylinerules` file defines – summaries, code snippets, next steps, project state, etc.
2. **Context Window Awareness:**
    - **Tracking:** skyline internally tracks the percentage of its available context window currently being used during a task.
    - **Visibility:** This information is visible in the `environment_details` provided to skyline in its prompt.

#### Using the `/newtask` Slash Command

As a quick alternative to skyline suggesting the `newtask` tool or defining complex rules, you can directly initiate the process using a Slash Command.

-   **How:** Simply type `/newtask` in the chat input field.
-   **Action:** skyline will propose creating a new task, typically suggesting context based on the current session (similar to its default behavior when using the tool). You will still get the `ask_followup_question` prompt to confirm and potentially modify the context before the new task is created.
-   **Benefit:** Provides a fast, user-initiated way to leverage the `new_task` functionality for branching explorations or managing long sessions without waiting for skyline to suggest it.

#### Default Behavior (Without `.skylinerules`)

By default, without specific `.skylinerules` dictating its behavior:

-   **Tool Availability:** The `new_task` tool exists, and skyline _can_ choose to use it.
-   **Context Awareness:** skyline _is_ aware of its context usage percentage.
-   **No Automatic Trigger:** skyline **will not** automatically initiate a task handoff _solely_ based on context usage reaching a specific percentage (like 50%). The decision to suggest using `new_task` comes from the AI model's reasoning based on the overall task progress and prompt instructions.
-   **Basic Context Preloading:** If `new_task` is used without specific rules defining the `<context>` block structure, skyline will attempt to preload relevant information based on its current understanding (e.g., a basic summary of progress and next steps), but this may be less comprehensive than a rule-driven approach.

#### The Power of `.skylinerules`: Enabling Custom Workflows

While the core capabilities exist by default, the true power, automation, and customization emerge when you combine `new_task` and context awareness with custom workflows defined in `.skylinerules`. This allows you to precisely control _when_ and _how_ skyline manages context and task continuity.

Key benefits of using `.skylinerules` with `new_task`:
...
-   **Intelligent Breakpoints:** Instruct skyline via rules to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.skylinerules` to have skyline automatically create a new task via `new_task` upon completing each subtask, preloading the context for the _next_ subtask.
-   **Custom Context Packaging:** Mandate the exact structure and content of the `<context>` block in `.skylinerules` for highly detailed and consistent handoffs (see example below).
...
A common workflow, **driven by specific `.skylinerules` like the example below**, involves these steps:

1. **Trigger Identification (Rule-Based):** skyline monitors for handoff points defined in the rules (e.g., context usage > 50%, task completion).
2. **User Confirmation:** skyline uses `ask_followup_question` to propose creating a new task, often showing the intended context defined by the rules.
...
3. **User Control:** You can approve, deny, or ask skyline to modify the context before the new task is created.
4. **Context Packaging (`new_task` Tool):** If approved, skyline uses `new_task`, packaging the context according to the structure mandated by the `.skylinerules`.
...
The effectiveness of rule-driven handoffs depends heavily on how `.skylinerules` define the `<context>` block. A comprehensive structure often includes:
...
The flexibility of `new_task` combined with `.skylinerules` opens up many possibilities:
...
-   **Intelligent Breakpoints:** Instruct skyline to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.skylinerules` to have skyline automatically create a new task via `new_task` upon completing each subtask.
...
Experimenting with `.skylinerules` is encouraged to discover workflows that best suit your needs!

#### Example `.skylinerules`: Task Handoff Strategy Guide

Below is an example `.skylinerules` file focused specifically on using `new_task` for context window management. **Remember, this is just one specific strategy; the core `new_task` tool can be used differently with other custom rules.**
...
-   **User Approval:** You always have control, approving the handoff and having the chance to modify the context skyline proposes to carry forward.
-   **Flexibility:** The core `new_task` tool is a flexible building block. Experiment with `.skylinerules` to create workflows that best suit your needs, whether for strict context management, task decomposition, or other creative uses.
```
````

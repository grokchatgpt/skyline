...
description: "Remote browser support allows skyline to utilize a remote Chrome instance, leveraging authentication tokens and session cookies relevant to certain web development test cases."
icon: globe-pointer
---

The Remote Browser feature in skyline allows the AI assistant to interact with web content directly through a controlled browser instance. This enables several powerful capabilities:
...
## Remote Browser in skyline

### What is Remote Browser?

Remote Browser allows skyline to view and interact with websites directly. This feature enables skyline to:
...
You can ask skyline to use the browser with simple instructions:
...
skyline can only use one browser at a time. If you want to visit a different website, you can either:

-   Ask skyline to navigate to a new URL within the same browser session
-   Ask skyline to close the current browser and open a new one

#### Browser Must Be Closed Before Using Other Tools

If you want skyline to edit files or run commands after using the browser, you must first ask it to close the browser:
...
The browser has a fixed viewport size (900x600 pixels by default), similar to a small laptop screen. skyline will share screenshots after each action so you can see exactly what it sees.

#### Console Logs

skyline captures browser console logs, which can be helpful for debugging web applications. These logs are included with each screenshot.

### Common Use Cases

-   **Web Development**: Test your websites and web applications
-   **UI/UX Review**: Get feedback on website design and usability
-   **Content Research**: Have skyline browse websites to gather information
...
-   **If the browser seems stuck**: Ask skyline to close the browser and try again
...
#### Configure skyline in VS Code:

1. Open VS Code settings
2. Search for "skyline: Chrome Executable Path"
3. Set the value to the path of your Chrome executable (e.g., `C:\Program Files\Google\Chrome\Application\chrome.exe`)

skyline should now be able to use the Remote Browser feature from within WSL.

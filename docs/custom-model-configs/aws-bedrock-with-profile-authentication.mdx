...
description: "Learn how to configure AWS Bedrock to use AWS Profiles for authentication with skyline, focusing on SSO/Federated roles for secure access."
---

### Overview

skyline offers the option of utilizing AWS credentials or AWS profiles to access AWS Bedrock services. SSO/Federated roles are suggested over Legacy IAM configuration; this guide describes how to configure your environment so that skyline uses SSO roles for authentication.
...
3. Continue configuring your profile using [the `aws configure sso` CLI wizard](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sso.html#cli-configure-sso-configure)
    ...
    - Note which profile name you attach to your AWS account, this is needed to configure skyline in the following steps

4. If you haven't already done so, install VSCode and the skyline extension. Consult the [Getting Started](/getting-started) page for guidance.

5. Open the skyline extension, then click on the settings button ⚙️ to select your API Provider.
...
<Frame>
	<img
		src="https://storage.googleapis.com/skyline_public_images/docs/assets/skyline-aws-setup-markup%20(1).png"
		alt="AWS Bedrock configuration in skyline settings showing profile authentication setup"
	/>
</Frame>

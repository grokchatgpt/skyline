...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "skyline-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .skylinerules/workflows directory specifically", async () => {
		// Create a test directory structure
		const skylinerulesDirTest = path.join(tmpDir, "skylinerules-test")
		const skylinerulesDirPath = path.join(skylinerulesDirTest, ".skylinerules")

		// Create .skylinerules directory and root files
		await fs.mkdir(skylinerulesDirPath, { recursive: true })
		await fs.writeFile(path.join(skylinerulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(skylinerulesDirPath, "settings.js"), "// settings")

		// Create .skylinerules/other directory and files
		const otherDirPath = path.join(skylinerulesDirPath, "other")
		...
		// Create .skylinerules/workflows directory and files
		const workflowsDirPath = path.join(skylinerulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(skylinerulesDirPath)
		...
		const filteredFiles = await readDirectory(skylinerulesDirPath, [[".skylinerules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(skylinerulesDirPath, "config.json"),
			path.resolve(skylinerulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(skylinerulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(skylinerulesDirPath, "config.json"), path.resolve(skylinerulesDirPath, "settings.js")]

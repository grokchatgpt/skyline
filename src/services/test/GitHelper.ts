...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".skyline_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "skyline Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "skyline@example.com"], { cwd: workspacePath })

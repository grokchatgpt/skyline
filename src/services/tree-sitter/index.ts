...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = skylineIgnoreController ? skylineIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, skylineIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (skylineIgnoreController && !skylineIgnoreController.validateAccess(filePath)) {
		return null
	}

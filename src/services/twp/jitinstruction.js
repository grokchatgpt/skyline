...
class JITInstructionManager {
	...
	_loadJITInstructionContent(messages = []) {
		...
		try {
			this.jitInstructionContent = fs.readFileSync(path.join(process.cwd(), promptFile), "utf8")
	...
	updateWindowUsageWithFinalCount(windowedMessages, conversationState, conversationId, botType = "anthropicapi") {
		...
		console.log(
			`[JITInstructionManager] TOKEN COUNT: system=${systemPromptTokens}, messages=${messagesTokens}, total=${totalTokens}/${this.maxWindowSize} (${percentage}%)`,
		)
		...
		const shouldTriggerJIT =
			this.config.JITinstruction && percentage >= this.config.JITinstruction.threshold && this.jitInstructionContent
		...
		if (shouldTriggerJIT) {
			...
			console.log(
				`[JITInstructionManager] TWP MCP FLOW: Preserved LastUserMessage (${userMessageTokens} tokens) and replaced with enhanced JIT + register mapping (${enhancedJITTokens} tokens)`,
			)
			...
			if (pattern && pattern.detectionText && userMessage.content && userMessage.content.includes(pattern.detectionText)) {
				...
				console.log(
					`[JITInstructionManager] Window usage updated: ${totalTokens}/${this.maxWindowSize} tokens (${percentage}%)`,
				)
			}
		}
	...
	truncateUserMessageIfNeeded(windowedMessages, conversationId, botType = "anthropicapi") {
		...
		console.log(
			`[JITInstructionManager] Token budget check: userMsg=${currentUserMessageTokens}, available=${availableForUserMessage}, others=${otherMessagesTokens}`,
		)
		...
		console.log(
			`[JITInstructionManager] User message truncated: ${currentUserMessageTokens} → ${countTokens(truncatedContent, botType)} tokens`,
		)
	...
	truncatePreviousAssistantMessage(windowedMessages, assistantIndex, tokensToFree, botType) {
		...
		console.log(
			`[JITInstructionManager] Truncated assistant message: ${originalTokens} → ${finalTokens} tokens (saved ${tokensSaved})`,
		)

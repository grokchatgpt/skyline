...
class TokenWindowManager {
	constructor(config = {}) {
		...
		this.oversizedConfig = this.config.oversizedMessageHandling || {
			...
			instructionTemplate:
				"[TRUNCATED - {originalTokens} tokens saved to {filepath}. Use grep to search for patterns, or 'tail -50 {filepath}' to see final output (sed, awk, wc, head and many other tools can be used instead of read_file to avoid subsequent truncation)]",
		}

		this.OVERSIZED_THRESHOLD = Math.floor(this.MAX_WINDOW_SIZE * (this.oversizedConfig.thresholdPercent / 100))

		this._logTrace(
			`Oversized message handling: ${this.oversizedConfig.enabled ? "enabled" : "disabled"}, threshold: ${this.OVERSIZED_THRESHOLD} tokens`,
		)
	}
	...
	processMessage(message) {
		...
		return this.processClientRequestWithBotId(conversationId, { messages: message.content }, message.destination)
	}
	...
	_parseUltraSimpleRecache(paramsString) {
		...
		for (const token of tokens) {
			...
			if (rangeMatch) {
				...
				console.log(
					`[TokenWindowManager] Added range ${start}-${end}: [${Array.from({ length: end - start + 1 }, (_, i) => start + i).join(", ")}]`,
				)
				continue
			}
		...
		console.log(
			`[TokenWindowManager] ULTRA-SIMPLE RECACHE complete: ${uniquePositions.length} positions [${uniquePositions.join(", ")}]`,
		)
	...
	_parseCacheProgrammingCommand(cacheReadArg, cacheMessagesWriteArg, cacheSystem2WriteArg) {
		...
		console.log(
			`[TokenWindowManager] ULTRA-LAX parsing: cache_read="${cacheReadArg}", cache_messages_write="${cacheMessagesWriteArg}", cache_system2_write="${cacheSystem2WriteArg}"`,
		)
		...
		if (messagesWriteTrimmed === "") {
			...
			console.log(
				`[TokenWindowManager] Forced roles: ${parsedMessagesWrite.map((entry) => `${entry.role === "user" ? "U" : "A"}${entry.position}`).join(", ")}`,
			)
		}
		...
		console.log(
			`[TokenWindowManager] ULTRA-LAX parsing complete: cache_read=${parsedCacheRead.type}, messages_write=${parsedMessagesWrite.length} entries, system2=${parsedSystem2.type}`,
		)
	...
	_detectMCPRestoreCommandWithoutValidation(content) {
		...
		const xmlMatch = content.match(
			/<use_mcp_tool>\s*<server_name>tokenwindow-local<\/server_name>\s*<tool_name>recache_message_array<\/tool_name>\s*<arguments>\s*({[^}]*})\s*<\/arguments>\s*<\/use_mcp_tool>/i,
		)
		...
		const jsonRpcMatch = content.match(
			/\{"jsonrpc":\s*"2\.0",\s*"method":\s*"tools\/call",\s*"params":\s*\{\s*"name":\s*"tokenwindow-local__recache_message_array",\s*"arguments":\s*({[^}]*})\s*\},\s*"id":\s*\d+\}/i,
		)
		...
		if (state && state.registers) {
			...
			if (invalidPositions.length > 0) {
				...
				if (state.registers && state.registers.length > 0) {
					const registerSummary = state.registers
						.slice(0, 10)
						.map((reg, i) => {
			...
			if (rangePlaceholderPositions.length > 0) {
				console.log(
					`[TokenWindowManager] Found ${rangePlaceholderPositions.length} placeholders in ranges (allowed): [${rangePlaceholderPositions.join(", ")}]`,
				)
			}

			if (individualPlaceholderPositions.length > 0) {
				console.log(
					`[TokenWindowManager] Found ${individualPlaceholderPositions.length} individually selected placeholders (error): [${individualPlaceholderPositions.join(", ")}]`,
				)
	...
	_handleMCPError(conversationId, errorMessage) {
		...
		console.log(
			`[TokenWindowManager] MCP Error streak: ${state.errorStreak}, saved ${state.lastUserMessageStack.length} LastUserMessages`,
		)
	...
	_stripRestoreCommand(content) {
		...
		cleanedContent = cleanedContent.replace(
			/\{"jsonrpc":\s*"2\.0",\s*"method":\s*"tools\/call",\s*"params":\s*\{\s*"name":\s*"tokenwindow-local__recache_message_array",[\s\S]*?\},\s*"id":\s*\d+\}/gi,
			"",
		)
	...
	_applyUltraSimpleRecache(conversationId, restoreCommand, latestAssistantMessage, currentMessages) {
		...
		if (state.lastUserMessage) {
			...
			console.log(
				`[TokenWindowManager] Built complete array: ${newRegisters.length} selected + 1 assistant + 1 preserved user = ${completeArray.length} total`,
			)
		...
		console.log(
			`[TokenWindowManager] AFTER ULTRA-SIMPLE RECACHE: ${state.registers.length} registers, ultra-simple cache complete`,
		)
	...
	_applyRecacheRestore(conversationId, restoreCommand, latestAssistantMessage, currentMessages) {
		...
		console.log(
			`[TokenWindowManager] APPLY EMERGENCY RECACHE: foundation=${foundation.type}, append=${append.length} entries`,
		)
		...
		if (foundation.type === "range") {
			...
			if (safeStart <= safeEnd && safeStart >= 1) {
				...
				console.log(
					`[TokenWindowManager] Added foundation range: ${safeStart}-${safeEnd} (${safeEnd - safeStart + 1} messages)`,
				)
			} else {
				console.log(
					`[TokenWindowManager] Foundation range ${foundation.start}-${foundation.end} clamped to empty (max: ${state.registers.length})`,
				)
			}
		} else {
		...
		if (state.lastUserMessage) {
			...
			console.log(
				`[TokenWindowManager] Built complete array: ${newRegisters.length} foundation+append + 1 assistant + 1 preserved user = ${completeArray.length} total`,
			)
	...
	_forceUserAssistantOrdering(registers, conversationId) {
		...
		for (let i = 0; i < registers.length; i++) {
			...
			if (register.role === expectedRole) {
				...
				console.log(
					`[TokenWindowManager] Added ${register.role} at position ${currentPosition} (matches expected ${expectedRole})`,
				)
				...
				console.log(
					`[TokenWindowManager] Added ${register.role} at position ${currentPosition} (ANTI-CONSECUTIVE: skipping placeholder after previous placeholder)`,
				)
			} else {
				...
				console.log(
					`[TokenWindowManager] Inserted fake ${fakeRole} at position ${currentPosition} (expected ${expectedRole})`,
				)
		...
		if (result.length % 2 === 0) {
			...
			if (!lastWasPlaceholder) {
				...
				console.log(
					`[TokenWindowManager] ANTI-CONSECUTIVE: Skipping final placeholder because last message was already placeholder`,
				)
			}
		}
		...
		for (let i = 1; i < result.length; i++) {
			if (result[i].fake && result[i - 1].fake) {
				...
				console.error(
					`[TokenWindowManager] This violates the core rule - there should NEVER be two placeholders in a row`,
				)
			}
		}
		...
		console.log(
			`[TokenWindowManager] Content preserved: ${realCount} real messages, ${fakeCount} fake messages added (NEVER consecutive)`,
		)
	...
	_handleOversizedMessage(register, conversationId) {
		...
		if (messageTokens > this.OVERSIZED_THRESHOLD) {
			console.log(
				`[TokenWindowManager] Oversized message detected: ${messageTokens} tokens > ${this.OVERSIZED_THRESHOLD} threshold`,
			)
	...
	_logOversizedMessage(conversationId, messageId, originalTokens, filepath) {
		try {
			...
			const logEntry =
				`[${timestamp}] OVERSIZED_MESSAGE_OFFLOAD for ${conversationId}\n` +
	...
	_updateRegistersFromMessages(conversationId, messages) {
		...
		for (const message of cleanedMessages) {
			...
			console.log(
				`[TokenWindowManager] Added new message as register ${newId} (${message.role}) - context window usage stripped`,
			)
		}
		...
		console.log(
			`[TokenWindowManager] Window now has ${state.registers.length} total registers for conversation ${conversationId}`,
		)
	}
	...
	_updateWindowUsage(windowedMessages, conversationId, botType = "anthropicapi") {
		...
		if (
			...
			console.log(
				`[TokenWindowManager] JIT instruction injected: ${newTotalTokens}/${this.MAX_WINDOW_SIZE} tokens (${newPercentage}%) - Context Window Usage stripped for cache safety`,
			)
		} else {
			console.log(
				`[TokenWindowManager] Context window calculated: ${totalTokens}/${this.MAX_WINDOW_SIZE} tokens (${percentage}%) - Context Window Usage stripped for cache safety`,
			)
		}
	...
	_injectJITInstruction(windowedMessages, conversationId, latestUserIndex) {
		...
		console.log(
			`[TokenWindowManager] JIT instruction injected at index ${latestUserIndex} for conversation ${conversationId}${state.currentMCPError ? " (with error context)" : ""}`,
		)
		...
		if (this.config.JITinstruction && this.jitManager.jitInstructionContent) {
			for (let i = 0; i < modifiedMessages.length; i++) {
				...
				if (
					message.role === "user" &&
					message.content &&
					message.content.includes(this.jitManager.jitInstructionContent)
				) {
			...
			for (let i = jitInjectionIndex; i < modifiedMessages.length; i++) {
				...
				if (message.content && typeof message.content === "string") {
					...
					if (
						...
						this.config.JITinstruction &&
						this.config.JITinstruction.assistantCleaning
					) {
	...
	_applyCacheProgrammingNeuralyzer(content) {
		...
		cleanedContent = cleanedContent.replace(/^\s*-?\s*Messages?\s+\d+/gim, "") // Remove "- Messages 9-10"
		cleanedContent = cleanedContent.replace(/^\s*-?\s*Message\s+\d+/gim, "") // Remove "- Message 1"
		...
		try {
			...
			if (conversationId) {
				try {
					...
					console.log(
						`[TokenWindowManager] Could not check conversation originator: ${error.message}, defaulting to twp_bak.txt`,
					)
					process.exit(1)
				}
			} else {
			...
			this.twpSystemPrompt = fs.readFileSync(path.join(process.cwd(), "data/config/prompts", promptFile), "utf8")
	...
	getTokenWindowForTransform(conversationId) {
		...
		const transformedMessages = this.jitManager.updateWindowUsageWithFinalCount(
			windowedMessages,
			state,
			conversationId,
			"anthropicapi",
		...
		console.log(
			`[TokenWindowManager] Returning ${transformedMessages.length} windowed messages for conversation ${conversationId}`,
		)
		...
		if (state.system2Content) {
			...
			console.log(
				`[TokenWindowManager] Built dual-block system prompt with System2 content for conversation ${conversationId}`,
			)
		}
	...
	_updateCacheBreakpoint(conversationId, oldBreakpoint, restoreCommand) {
		...
		console.log(
			`[TokenWindowManager] Cache stats updated: cache_creation=${cacheStats.cache_creation_input_tokens}, cache_read=${cacheStats.cache_read_input_tokens}`,
		)
	}
	...
	_countTokensInRange(registers, startIndex, endIndex) {
		...
		// Range validation
		if (typeof startIndex !== "number" || typeof endIndex !== "number" || startIndex < 1 || endIndex < startIndex) {
	...
	getTokenWindowForTransformWithBotId(conversationId, botId = null) {
		console.log(
			`[TokenWindowManager] Getting token window for transform with botId, conversation ${conversationId}, bot ${botId}`,
		)
			...
			try {
				...
				if (conversation && conversation.type === "shared_chat" && conversation.participants) {
					...
					if (botParticipant && botParticipant.role) {
						console.log(
							`[TokenWindowManager] Enhancing system prompt for ${botId} with shared chat role: ${botParticipant.role}`,
						)
						...
						console.log(
							`[TokenWindowManager] System prompt enhanced with shared chat role for conversation ${conversationId}`,
						)
					}
				}
			} catch (error) {
	...
	getCacheStatsForResponse(conversationId) {
		...
		console.log(
			`[TokenWindowManager] Returning cache stats for ${conversationId}: cache_creation=${state.lastCacheStats.cache_creation_input_tokens}, cache_read=${state.lastCacheStats.cache_read_input_tokens}`,
		)

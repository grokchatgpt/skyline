...
class TWPEfficiencyScorer {
  ...
  _calculateOutputScore(messageAnalysis, modeConfig, mode) {
    ...
    if (messageAnalysis.thinkingBeforeToolTokens > modeConfig.max_thinking_before_tool) {
      score = 'INEFFICIENT';
      details.push(`Excessive pre-tool thinking: ${messageAnalysis.thinkingBeforeToolTokens} > ${modeConfig.max_thinking_before_tool} tokens`);
    }

    if (messageAnalysis.twpExplanationTokens > modeConfig.max_twp_explanation_tokens) {
      score = 'INEFFICIENT';
      details.push(`Long TWP explanations: ${messageAnalysis.twpExplanationTokens} > ${modeConfig.max_twp_explanation_tokens} tokens`);
    }
  ...
  _calculateWindowGrowthRate(windowState) {
    ...
    const currentSize = (windowState.messageRegisters?.length || 0) + 
                       (windowState.thoughtRegisters?.length || 0);

...
export class skylineHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		for await (const chunk of stream) {
			...
			if ("error" in chunk) {
				...
				console.error(`skyline API Error: ${error?.code} - ${error?.message}`)
				// Include metadata in the error message if available
				const metadataStr = error.metadata ? `\nMetadata: ${JSON.stringify(error.metadata, null, 2)}` : ""
				throw new Error(`skyline API Error ${error.code}: ${error.message}${metadataStr}`)
			}
	...
	async getApiStreamUsage(): Promise<ApiStreamUsageChunk | undefined> {
		if (this.lastGenerationId) {
			try {
				...
				// ignore if fails
				console.error("Error fetching skyline generation details:", error)
			}
		}
		return undefined
	}

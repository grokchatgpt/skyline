import { skylineIgnoreController } from "./skylineIgnoreController"
...
describe("skylineIgnoreController", () => {
	let tempDir: string
	let controller: skylineIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .skylineignore file
		await fs.writeFile(
			path.join(tempDir, ".skylineignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new skylineIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .skylineignore file", async () => {
			const result = controller.validateAccess(".skylineignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .skylineignore", async () => {
			// Create a new .skylineignore with comments
			await fs.writeFile(
				path.join(tempDir, ".skylineignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .skylineignore gracefully", async () => {
			// Create a new controller in a directory without .skylineignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new skylineIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .skylineignore", async () => {
			await fs.writeFile(path.join(tempDir, ".skylineignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .skylineignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".skylineignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .skylineignore
			controller = new skylineIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .skylineignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .skylineignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".skylineignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".skylineignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))

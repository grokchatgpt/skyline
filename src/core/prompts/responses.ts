...
import { skylineIgnoreController, LOCK_TEXT_SYMBOL } from "../ignore/skylineIgnoreController"
...
export const formatResponse = {
	...
	skylineIgnoreError: (path: string) =>
		`Access to ${path} is blocked by the .skylineignore file settings. You must try to continue in the task without using this file, or ask the user to update the .skylineignore file.`,
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		files: string[],
		didHitLimit: boolean,
		skylineIgnoreController?: skylineIgnoreController,
	): string => {
		const sorted = files
			.map((file) => {
			...
			// Sort so files are listed under their respective directories to make it clear what files are children of what directories. Since we build file list top down, even if file list is truncated it will show directories that skyline can then explore further.
			.sort((a, b) => {
		...
		const skylineIgnoreParsed = skylineIgnoreController
			? sorted.map((filePath) => {
					// path is relative to absolute path, not cwd
					// validateAccess expects either path relative to cwd or absolute path
					// otherwise, for validating against ignore patterns like "assets/icons", we would end up with just "icons", which would result in the path not being ignored.
					const absoluteFilePath = path.resolve(absolutePath, filePath)
					const isIgnored = !skylineIgnoreController.validateAccess(absoluteFilePath)
					if (isIgnored) {
		...
		if (didHitLimit) {
			return `${skylineIgnoreParsed.join(
				"\n",
			)}\n\n(File list truncated. Use list_files on specific subdirectories if you need to explore further.)`
		} else if (skylineIgnoreParsed.length === 0 || (skylineIgnoreParsed.length === 1 && skylineIgnoreParsed[0] === "")) {
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	skylineIgnoreInstructions: (content: string) =>
		`# .skylineignore\n\n(The following is provided by a root-level .skylineignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${content}\n.skylineignore`,

	skylineRulesGlobalDirectoryInstructions: (globalskylineRulesFilePath: string, content: string) =>
		`# .skylinerules/\n\nThe following is provided by a global .skylinerules/ directory, located at ${globalskylineRulesFilePath.toPosix()}, where the user has specified instructions for all working directories:\n\n${content}`,

	skylineRulesLocalDirectoryInstructions: (cwd: string, content: string) =>
		`# .skylinerules/\n\nThe following is provided by a root-level .skylinerules/ directory where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,

	skylineRulesLocalFileInstructions: (cwd: string, content: string) =>
		`# .skylinerules\n\nThe following is provided by a root-level .skylinerules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,

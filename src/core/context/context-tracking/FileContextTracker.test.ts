...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.skyline_read_date).to.be.a("number")
		expect(fileEntry.skyline_edit_date).to.be.null
	})

	it("should add a record when a file is edited by skyline", async () => {
		const filePath = "src/test-file.ts"

		await tracker.trackFileContext(filePath, "skyline_edited")
		...
		expect(activeEntry.record_source).to.equal("skyline_edited")
		expect(activeEntry.skyline_read_date).to.be.a("number")
		expect(activeEntry.skyline_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				skyline_read_date: Date.now() - 1000, // 1 second ago
				skyline_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("skyline_edited")
	})
	...
	it("should not track skyline edits as user edits", async () => {
		const filePath = "src/test-file.ts"

		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by skyline
		tracker.markFileAsEditedByskyline(filePath)

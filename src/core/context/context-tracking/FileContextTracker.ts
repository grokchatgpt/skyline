...
// If a user modifies a file outside of skyline, the context may become stale and need to be updated.
// We do not want skyline to reload the context every time a file is modified, so we use this class merely
// to inform skyline that the change has occurred, and tell skyline to reload the file before making
// any changes to it. This fixes an issue with diff editing, where skyline was unable to complete a diff edit.
// a diff edit because the file was modified since skyline last read it.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are pass to skyline via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of skyline, we detect and track this change to prevent stale context.
export class FileContextTracker {
	...
	private recentlyEditedByskyline = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.onDidChange(() => {
			if (this.recentlyEditedByskyline.has(filePath)) {
				this.recentlyEditedByskyline.delete(filePath) // This was an edit by skyline, no need to inform skyline
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform skyline
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to skyline via a tool, mention, or edit.
	async trackFileContext(filePath: string, operation: "read_tool" | "user_edited" | "skyline_edited" | "file_mentioned") {
		try {
			const cwd = this.getCwd()
			if (!cwd) {
				return
	...
	async addFileToFileContextTracker(
		...
		try {
			...
			let newEntry: FileMetadataEntry = {
				...
				skyline_read_date: getLatestDateForField(filePath, "skyline_read_date"),
				skyline_edit_date: getLatestDateForField(filePath, "skyline_edit_date"),
			...
			switch (source) {
				...
				// skyline_edited: skyline has edited the file
				case "skyline_edited":
					newEntry.skyline_read_date = now
					newEntry.skyline_edit_date = now
					break

				// read_tool/file_mentioned: skyline has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.skyline_read_date = now
					break
	...
	// Marks a file as edited by skyline to prevent false positives in file watchers
	markFileAsEditedByskyline(filePath: string): void {
		this.recentlyEditedByskyline.add(filePath)
	}

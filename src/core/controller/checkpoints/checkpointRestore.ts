...
export async function checkpointRestore(controller: Controller, request: CheckpointRestoreRequest): Promise<Empty> {
	...
	if (request.number) {
		...
		await pWaitFor(() => controller.task?.isInitialized === true, {
			...
			console.error("Failed to init new skyline instance")
		})

		// NOTE: cancelTask awaits abortTask, which awaits diffViewProvider.revertChanges, which reverts any edited files, allowing us to reset to a checkpoint rather than running into a state where the revertChanges function is called alongside or after the checkpoint reset
		await controller.task?.restoreCheckpoint(request.number, request.restoreType as skylineCheckpointRestore, request.offset)
	}
	return {}
}

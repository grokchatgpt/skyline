...
export function registerTaskCommands(context: vscode.ExtensionContext, controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("skyline.dev.createTestTasks", async () => {
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): skylineMessage[] {
	...
	const messages: skylineMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in skyline.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",

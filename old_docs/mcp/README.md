# skyline and Model Context Protocol (MCP) Servers: Enhancing AI Capabilities
...
This document explains Model Context Protocol (MCP) servers, their capabilities, and how skyline can help build and use them.
...
skyline simplifies the building and use of MCP servers through its AI capabilities.

### Building MCP Servers

-   **Natural language understanding:** Instruct skyline in natural language to build an MCP server by describing its functionalities. skyline will interpret your instructions and generate the necessary code.
-   **Cloning and building servers:** skyline can clone existing MCP server repositories from GitHub and build them automatically.
-   **Configuration and dependency management:** skyline handles configuration files, environment variables, and dependencies.
-   **Troubleshooting and debugging:** skyline helps identify and resolve errors during development.

### Using MCP Servers

-   **Tool execution:** skyline seamlessly integrates with MCP servers, allowing you to execute their defined tools.
-   **Context-aware interactions:** skyline can intelligently suggest using relevant tools based on conversation context.
-   **Dynamic integrations:** Combine multiple MCP server capabilities for complex tasks. For example, skyline could use a GitHub server to get data and a Notion server to create a formatted report.
